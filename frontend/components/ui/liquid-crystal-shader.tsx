...
// Blue-only shader: colours stay in navy→bright-blue range matching the site palette
...
void main() {
  ...
  // Glow intensity from distance field
  float glow = 0.012 / (abs(d) + 0.002);

  // Blue-only palette: r stays very low, g stays low-mid, b is dominant
  // This matches the site's blue-900 → blue-300 range
  float t = u_time * 0.3;
  float r = 0.08 + 0.08 * cos(t * 0.7 + uv.x * 2.0);
  float g = 0.20 + 0.20 * cos(t * 0.5 + uv.y * 1.5 + 1.0);
  float b = 0.60 + 0.40 * cos(t * 0.4 + uv.x - uv.y + 2.0);

  vec3 col = clamp(vec3(r, g, b) * glow, 0.0, 1.0);
...
const LiquidCrystalBackground: FC<LiquidCrystalProps> = ({
  ...
  useEffect(() => {
    ...
    if (!vs || !fs) { setError("Shader compile error"); return; }
  ...
  return (
    ...
    >
      ...
      {error && (
        // Fallback: plain blue gradient if WebGL2 unavailable
